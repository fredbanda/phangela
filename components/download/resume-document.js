...
try {
  Font.register({
    ...
    fonts: [
      ...
      {
        src: 'https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuOKfAZ9hiJ-Ek-_EeA.woff2',
        fontWeight: 'bold',
      },
    ],
  });
...
const ResumePDFDocument = ({ resume }) => {
  ...
  console.log(
    'ResumePDFDocument: Personal info keys:',
  ...
  const resumeData = {
    ...
    location: (
      ...
      .toString()
      .trim(),
    city: (personalInfo.city || resume.city || resume.location || '')
      .toString()
      .trim(),
    linkedin: (personalInfo.linkedin || resume.linkedin || '')
      .toString()
      .trim(),
    ...
    experience: Array.isArray(resume.experience)
      ...
      : Array.isArray(resume.jobs)
      ? resume.jobs
      : [],
    education: Array.isArray(resume.education)
      ? resume.education
      : Array.isArray(resume.educationHistory)
    ...
    skills: Array.isArray(resume.skills)
      ...
      : Array.isArray(resume.coreSkills)
      ? resume.coreSkills
      : [],
  };

  console.log(
    'ResumePDFDocument: Processing resume with name:',
    resumeData.name
  );
  console.log(
    'ResumePDFDocument: Experience items:',
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.jobTitle}>{resumeData.job || ''}</Text>
            ...
            <Text style={styles.description}>
              {resumeData.skills
                .map((skill, index) => {
                  ...
                  return skillText && skillText.length > 0
                    ? `• ${skillText}`
                    : '';
                })
        ...
        {!resumeData.summary &&
          ...
          (!resumeData.skills || resumeData.skills.length === 0) && (
            <View style={styles.section}>
              <Text style={styles.description}>
                Resume content is being processed. Please ensure all sections
                are properly filled out in the resume builder.
              </Text>
            </View>
          )}
      </Page>
...
const ExperienceItem = ({ job, index }) => {
  ...
  if (
    ...
    endDate = (job.endDate || job.endYear || job.to || 'Present')
      .toString()
      .trim();
  ...
  const location = (job.location || job.city || job.address || '')
    .toString()
    .trim();
  ...
  console.log(
    `Experience ${index} - Description Field Value:`,
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      {bulletPoints && bulletPoints.length > 0 && (
        <View>
          {bulletPoints.length > 1 ? (
            ...
            <Text style={styles.description}>{bulletPoints[0]}</Text>
          )}
        </View>
      )}
          ...
          <View>
            {job.achievements
              .map((achievement, achIndex) => {
                const achText = stripHtml(
                  (achievement || '').toString().trim()
                );
      ...
      {job.bulletPoints &&
        Array.isArray(job.bulletPoints) &&
...
const EducationItem = ({ education }) => {
  const degree = (education.qualification || 'Degree').toString().trim();
  const school = (education.institution || 'Institution').toString().trim();
  const startDate = (education.startDate || 'Start').toString().trim();
  const endDate = (education.endDate || 'End').toString().trim();
  const address = (education.address || '').toString().trim();
  const city = (education.city || '').toString().trim();
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      ...
      {education.gpa && education.gpa.toString().trim().length > 0 && (
        <Text style={styles.description}>
          GPA: {education.gpa.toString().trim()}
        </Text>
      )}
